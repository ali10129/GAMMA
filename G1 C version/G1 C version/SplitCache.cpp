...
static void delta(uint dm, uint dk, uint dn, uint cmb, uint _bsize, uint _asso)
{
	...
	double MKN = M*K + K*N + M*N;
	double MK = M*K;
	double KN = K*N;
	double MN = M*N;

	int sizeA = (cmb * 1024) * (MK / MKN);
	int sizeB = (cmb * 1024) * (KN / MKN);
	int sizeC = (cmb * 1024) * (MN / MKN);
	...
	////////////Save results:///////////////////////////////////////////////////////////////////////////////////////////
	Cache::info(&A0, &A1, &A2, &B0, &B1, &B2, &C0, &C1, &C2, dm, dk, dn, cmb, _bsize, _asso);
}
